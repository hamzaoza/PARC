var index = (function (buffer, string_decoder, zlib, path$2, querystring, events, url, fs, http, stream, crypto, util, net) {
	'use strict';
	...
	path$2 = path$2 && path$2.hasOwnProperty('default') ? path$2['default'] : path$2;
	...
	var debug$1 = browser$1('body-parser:json');
	...
	var debug$2 = browser$1('body-parser:raw');
	...
	var debug$3 = browser$1('body-parser:text');
	...
	var debug$4 = browser$1('body-parser:urlencoded');
	var deprecate = browser('body-parser');
	...
	var debug$5 = browser$1('finalhandler');
	...
	var debug$6 = browser$1('express:router:layer');
	...
	function getCurrentNodeMethods() {
	  return require$$3$1.METHODS && require$$3$1.METHODS.map(function lowerCaseMethod(method) {
	...
	var debug$7 = browser$1('express:router:route');
	...
	var debug = browser$1('express:router');
	var deprecate = browser('express');
	...
	var debug$8 = browser$1('express:view');
	var path = path$2;
	...
	var debug$9 = browser$1('send');
	var deprecate$1 = browser('send');
	...
	var debug = browser$1('express:application');
	...
	var deprecate$2 = browser('express');
	var isIP = net.isIP;
	...
	var deprecate$3 = browser('express');
	...
	res.send = function send(body) {
	  ...
	  switch (typeof chunk) {
	    ...
	    case 'object':
	      if (chunk === null) {
	        chunk = '';
	      } else if (Buffer$7.isBuffer(chunk)) {
	        if (!this.get('Content-Type')) {
	          this.type('bin');
	        }
	      } else {
	  ...
	  if (chunk !== undefined) {
	    if (Buffer$7.isBuffer(chunk)) {
	      ...
	      len = Buffer$7.byteLength(chunk, encoding);
	    } else {
	...
	res.redirect = function redirect(url) {
	  ...
	  this.set('Content-Length', Buffer$7.byteLength(body));
	...
	var app = express$1();
	var port = process.env.PORT || 3000; // Use dist folder to display webpage.

	app.use(express$1.static("dist")); // Listen on port 3000 for webserver.
...
}(buffer, string_decoder, zlib, path$2, querystring, events, url, fs, http, stream, crypto, util, net));
